...
def c3():
    global input
    input ("VOUS AVEZ PASSER LA FORET GRACE À UN PORTAIL MAGIC (TKT C MAGIK)")
    input ("FACE À VOUS 7 ENFANTS SE PRÉSENTENT (ON S'EN FOUT DES NOMS)")
    c3c1 = input ("LE PREMIER VOUS TENT UN BOBON VOUS ACCEPTEZ ? > ")
    if c3c1 == "oui":
        input ("VOUS ÊTES MORT")
        input ("EN MÊME TEMP FAUT ÊTRE CON POUR MANGER UN BONBON DONNER PAR UN ENFANT LOUCHE")
    else :
        input("OUI C'EST UNE BONNE IDÉE TRÈS BON CHOIX")  
          
    input = ("LE DEUXIÊME Vous parle pas lol")
   
    c3c3 = input = ("LE TROISIEME VOUS FRAPPE VOULEZ VOUS VOUS VENGER")
    if c3c3 == "oui" :
        input = ("VOUS FRAPPER L'ENFANT IL TOMBE AU SOL")                       
        input =("DES ELFES, DES ORCS, DES GOBLINS,TOUTES LES RACES DE LA CONTRER DU CUE ENFAITES SORTENT DE DERRIERES LES ARBRES ET VOUS APLAUDISSENT POUR VOTRE PROUÈSSE")
    elif c3c3 == "non" :
        input ("AH LA VICTIME IL SE FAIT FRAPPER PAR UN ENFANT")
        input ("VOUS AVEZ DEBLOQUER LE SUCCÉS [VICTIME]")
    else :
        input ("Wesh")
    input ("LE QUATRIEME MONTE À UN ARBRE ")
    input ("VOUS LE REGARDER LENTEMENT MONTER À LARBRE")
    input ("IL SAUTE DE L'ARBRE EN CRIANT :")
    input ("1")
    input ("2")
    input ("3")
    input ("alkaida")
    input ("VOUS L'ESQUIVER DE PEU ET IL PART EN CRIANT DES INVOCATIONS SATANIQUES")
    input ("LE CINQUIEME DAB")
    c3c4 = input ("LE FRAPPER ? > ")
    if c3c4 == "oui":
        input ("CHE")
    else :
        input ("HEU BEN OK DOMMAGE MOI JE L'AURAIS FRAPPER A VOTRE PLACE")
        input ("ENFIN JE DIT CA JE DIT RIEN")
    input  ("LE SIXEME")
    c3c5 = input ("PFF CEST LONG BON LES 2 DERNIERS VOUS INDIQUE LA DIRECTION VOUS LES SUIVEZ ? > ")
    if c3c5 == "oui" :
        input ("VOUS LES SUIVEZ ET IL VOUS INVITE À RENTRER CHEZ EUX")
        c3c6 = input ("VOULEZ VOUS RENTREZ ? >")
        if c3c6 == "oui" :
            input ("VOUS RENTRER DANS LA MAISON")
            input ("VOTRE TÊTE CE METS À TOURNER")
            c3c7 = input ("SE PLANTER LA JAMBE AVEC UN COUTEAU (A),FAIRE UN DAB(B)")
            if c3c7 == "A" :
                global épée_actuelle
                input ("TRES BONNE IDEE! LA DOULEUR VOUS REVEILLE ET VOUS VOUS TROUVEZ FACE AU 2 ENFANTS AVEC DES COUTEAU QUE FAIRE")
                c3c8 = input ("ATTAQUER (A), FUIR (B)")
                if c3c8 == "A" and épée_actuelle == "épée_du_cue" :
                    input ("VOUS ATTAQUER AVEC VOTRE épée_du_cue")
                    input ("VOUS AVEZ TUER LES ENFANTS DE SATAN, VOUS AVEZ LIBERE LE MONDE DES ENFANTS DE SATAN")
                 
                    input ("VOUS AVEZ DEBLOQUER LE SUCCES [HERO]")
                    input ("VOTRE " + épée_actuelle + "AS PRIS DES DEGATS ATTENTION")
                    c4()
                elif c3c8 == "A" and épée_actuelle == "épée_de_bois_du_cue" :
                    input ("VOUS ATTAQUER AVEC VOTRE épée_de_bois_du_cue, ELLE EST EN RÉALITÉ EN LATEX, VOUS EFRAYEZ LES ENFANTS AVEC LE LATEX CA LEUR RAPELLE DE MAUVAIS SOUVENIR. MERCI GANDOULF LE VIEUX")
                    input ("BRAVO VOUS AVEZ VAINCU TOUT LES ENFANTS DE SATAN, A OUI J'AI OUBLIAIS DE VOUS DIRE QUE C'ETAIT LES ENFANTS DE SATAN")
                    c4()
                elif c3c8 == "A" and épée_actuelle == "rien" :
                    input ("VOUS AGITER FRENETIQUEMENT LES BRAS DANS LES AIRS")
                    input ("LES ENFANTS PRENNENT PEUR ET VOUS DONNE :")
                    input ("epee_de_fukin_satan")
                ...
                input ("SÉRIEUX UN DAB ???")
                input ("BEN TU SAIS QUOI TA GAGNER ALLER VAS DANS LE VILLAGE")
                c4() 
        else :
          input ("HOPE LA FUITE")
          c4()
